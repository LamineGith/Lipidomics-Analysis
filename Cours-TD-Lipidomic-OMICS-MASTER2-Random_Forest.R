...
#############################################################################################
####################  l'analyse du 28/08/2015 apres pamr ? s?lectionner 38 lipides
...
########################################################################################
########### dans le dossier "dataframesouriswork1" il faut s?lectionner que le foie  ###

dataframe <-subset(dataframesouriswork1, Tissue=="liver")
View(dataframe)


###########  analyse des 38 lipides trouv?s dans le foie apr?s pamr analyse pour threshold 2.301  #########
...
############# boxplots correspondat ? chaque variable   ##########
library("reshape2")
lipid <- data.frame[,c("groupe",usedVar)]#### remplacer groups par groupe>>>> qu'est que cela donne???
...
View(z)
x11()
lipid.pv <- pvclust(data1, nboot=1000)
plot(lipid.pv)
ask.bak <- par()$ask
par(ask=TRUE)
pvrect(lipid.pv)
print(lipid.pv, digits=4)###digits = nbre de chiffres apr?s la virgule
...
#### analyse en composante principal des r?sultats de "lipid" resultat de pamr
### 
...
#### analyse en composante principal des r?sultats de "lipid" resultat de pamr
### 
res.pcaa <- PCA(data1, scale.unit=TRUE, ncp=40, quali.sup=c(1:1), graph = FALSE)
sourisfoie <- HCPC(res.pcaa, nb.clust=3, cluster.CA="rows",order=TRUE,nb.par=3,proba=0.05,consol=TRUE, iter.max=10)
x11()
plotellipses(res.pcaa)

dimdesc(res.pcaa)$Dim.1$quanti[,1] # permet de s?parer les MCD des autres
...
### d?termine le meilleur "mtry" ca marche pas prend le deuxie
bestmtry <- tuneRF(z1, group, ntreeTry=12, stepFactor=1.5,improve=0.01, trace=TRUE, plot=TRUE, dobest=T)
### mtry=12

z2<-as.data.frame(z1)
### d?termine le meilleur "mtry"
bestmtry <- tuneRF(z2[,-14],z2[,14], ntreeTry=12, stepFactor=1.5,improve=0.01, trace=TRUE, plot=TRUE, dobest=T)
### mtry=12
...
##### Tracer les coordonn?es d'?chelle de la matrice de proximit? de Forest al?atoire ####################
### "PCA" des "groupe"= "Control", "NAFLD" et "NASH" en fonction de l'analyse Randome Forest "lipid.rf ###
##########################################################################################################

x11()
MDSplot(lipid.rf, dataframe$groups, palette=rep(1, 3), pch=as.numeric(dataframe$groups))   ### "groups" correspoind ? la colonne des 3 groupe "control", HFD et MCD
...
##### groupe= "Control", "NAFL1", "NAFL2", "NAFL3", "NASH" qui repr?sentent les "branches" #########
###### c'est pour ?a que le "MDS" qui en d?coule est plus jolie ####################################
...
########## Cette approche permet de s?lectionner le plus petit nombre de variables 
########## capablent de discriminer ? la foies les branches et les feuilles
########## ce qui permet de d?terminer du coup le OOB: Out of bag error rate.
...
z1  <- as.matrix(lipids) ##### on reprend la matrice cr?ee au d?but
...
####  lipides majeur apr?s Randome Forest (voir le graphique Variable Importnace"
...
sourisfoie32 <- HCPC(res.pcaa32, nb.clust=3, cluster.CA="rows",order=TRUE,nb.par=3,proba=0.05,consol=TRUE, iter.max=10)
dimdesc(res.pcaa32) ###permet de d?finir les dimension de la PCA pour lesquels les lipides son discriminant
dimdesc(res.pcaa32, axes=1:5) ###on regarde les dimension de la PCA de 1 ? 5
x11()
plotellipses(res.pcaa32, axis=c(1,2)) ### on repr?sente la PCA sur les dimension significative ici 1 et 3 d'apr?s le resultat dimdesc
...
############### 15 lipides commun foie et serum ? partir des 38 lipides foie et 38 lipides serum
...
sourisfoie15 <- HCPC(res.pcaa15, nb.clust=3, cluster.CA="rows",order=TRUE,nb.par=3,proba=0.05,consol=TRUE, iter.max=10)
dimdesc(res.pcaa15) ###permet de d?finir les dimension de la PCA pour lesquels les lipides son discriminant
dimdesc(res.pcaa15, axes=1:5) ###on regarde les dimension de la PCA de 1 ? 5
x11()
plotellipses(res.pcaa15, axis=c(1,2)) ### on repr?sente la PCA sur les dimension significative ici 1 et 3 d'apr?s le resultat dimdesc
...
model1
model2
summary(model)[[8]]#### permet d'extraire le R2 de la fonction lin?aire
summary(model1)[[8]]
summary(model2)[[8]]
anova(model)#### permet de tester R2 de la fonction lin?aire pour voir p
anova(model1)
anova(model2)
...
### etc..... avec toutes les varible ? tester 
...
detach(dataframesouriswork1)### on sort de la session, on se d?tache
rm(list=ls())### permet d'?liminer les ?l?ments upload?s lors d'une analyse pr?c?dente= session pr?c?dente
ls()### verification que la liste est vide ou dan RStudio que la fen?tre "Envoronnement est vide#
